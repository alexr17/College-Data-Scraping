  ...
  clean: function(dataSet, categories) {
    try {
      ...
      //for (var kk = 0; kk < dataSet.length; kk++) {
        //GPA
        if (gpaIndex != -1 && dataSet[gpaIndex] != null) {
          dataSet[gpaIndex] = getNumber(dataSet[gpaIndex], 4, 4.0); // 4.0 max value 4 digits (including the decimal)
        }

        //ACT
        if (actIndex != -1 && dataSet[actIndex] != null) {
          dataSet[actIndex] = getNumber(dataSet[actIndex], 2, 36); //36 max score 2 digits
        }
        
        //decision and specific college
        if (decIndex != -1 && dataSet[decIndex] != null/*&& objIndex != -1*/) {
          var dec = auxClean.editText(dataSet[decIndex].toLowerCase(), decisions); //get the decision
          //var otherText = dataSet[kk][decIndex].replace(dec, ""); //remove it from the text
          //dataSet[objIndex] = getSchool(otherText, dataSet[kk][objIndex], schools); //get the school
          dataSet[decIndex] = dec; //assign the decision
        }
        
        //ranking
        if (rankIndex != -1 && dataSet[rankIndex] != null) {
          dataSet[rankIndex] = getRank(dataSet[rankIndex].replace(/[^/% 0-9]/g, '')); //remove all non numbers, keeping ".","/", and "%"
        }

        //sat
        if (satIndex != -1 && dataSet[satIndex] != null) {
          var sat = getSATScore(dataSet[satIndex]);
          //console.log("SAT: " + sat);
          var total = Number(sat[0]) + Number(sat[1]) + Number(sat[2]);
          dataSet[satIndex] = sat.join() + ',' + total; 
        }
      //}
      return dataSet;
    } catch (err) {
